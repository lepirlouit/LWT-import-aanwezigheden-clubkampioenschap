...
const updateAanwezigheidRecord = async ({ niss, points })  => {
  ...
  SET Andere = Andere + 1,
    punten = REPLACE(CAST(CAST(replace(punten, ',', '.') AS DECIMAL(4)) + ? AS CHAR), '.', ','),
    bedrag2022 = REPLACE(CAST(CAST(replace(bedrag2022, ',', '.') AS DECIMAL(4,1)) + ? AS CHAR), '.', ','),
    totaalbedrag = REPLACE(CAST(CAST(replace(totaalbedrag, ',', '.') AS DECIMAL(4,1)) + ? AS CHAR), '.', ','),
    totaalpunten = REPLACE(CAST(CAST(replace(totaalpunten, ',', '.') AS DECIMAL(4)) + ? AS CHAR), '.', ',')
